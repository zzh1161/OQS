...
if _numba_available():
    ...
    def _strang_nl_step_kernel(
        ...
        for s in prange(n_states):
            _matvec_into(P, psi_prev[s], psi_half[s])

        for s in prange(n_states):
            for i in range(dim):
                psi_curr[s, i] = psi_half[s, i]

            _matvec_add_scaled(L, psi_half[s], psi_curr[s], z_n * dt)

            for k in range(rank):
                s_minus = idx_minus[s, k]
                if s_minus >= 0:
                    scale_minus = dt * idx_values[s, k] * Vn[k]
                    _matvec_add_scaled(L, psi_half[s_minus], psi_curr[s], scale_minus)

                s_plus = idx_plus[s, k]
                if s_plus >= 0:
                    scale_plus = -dt * lam[k] * Un[k]
                    _matvec_add_scaled(L_dag, psi_half[s_plus], psi_curr[s], scale_plus)
                    scale_nl = dt * lam[k] * Un[k] * L_dag_avg_n
                    _vec_add_scaled(psi_half[s_plus], psi_curr[s], scale_nl)

    @njit(cache=True, nogil=True)
    def _strang_nl_step_kernel_serial(
        psi_prev,
        psi_half,
        psi_curr,
        P,
...
class NMLRSSE_NL_1stOrderStrang:
    def __init__(
        self,
        Hs,
        L,
