...
if _numba_available():
    ...
    def _psi_step_kernel(
        ...
        for s in prange(n_states):
            ...
            for k in range(rank):
                ...
                if s_minus >= 0:
                    scale = dt * idx_values[s, k] * Vn[k]
                    _matvec_add_scaled(L, psi_prev[s_minus], psi_curr[s], scale)

                s_plus = idx_plus[s, k]
                if s_plus >= 0:
                    scale = -dt * lam[k] * Un[k]
                    _matvec_add_scaled(L_dag, psi_prev[s_plus], psi_curr[s], scale)
    ...
    def _psi_step_kernel_serial(
        ...
        """Serial variant for trajectory-level parallelization."""
        n_states, dim = psi_prev.shape
        rank = Vn.shape[0]
...
class NMLRSSE_forwardEuler:
    ...
    def psi_step(self, z, n):
        ...
        A = self.Id - 1j * self.Hs * self.dt + z[n] * self.L * self.dt
        for idx in self.idx_set:
